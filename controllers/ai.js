...
function extractFilters(message) {
  ...
  // ── Itinerary / trip planning detection ──
...
// Primary: grabs what comes after "trip to" / "plan a trip to"
// ─────────────────────────────────────────────
function extractDestinations(message) {
  // Most reliable: "Plan a X-day trip to DEST from/starting/for..."
  const tripTo = message.match(
  ...
  // Fallback: "trip to DEST", "travel to DEST", "visiting DEST"
...
//
// Priority order:
//  1. Exact listing name mentioned       → Mongo _id lookup
//  2. Explicit destination provided      → Mongo location/country regex
//     (passed from itinerary page)         + vector fallback
//  3. Price-only filter                  → Mongo price query
//  4. Any intent query (solo, beach…)    → Vector search (semantic)
//  5. Show all / no signal               → Listing.find({}) no cards
// ─────────────────────────────────────────────
...
async function fetchRelevantListings(userMessage, explicitDestination = null) {
  ...
  // Prefer explicit destination (sent from itinerary page) over regex extraction
  ...
  if (namedListings.length > 0) {
    ...
    return { listings: namedListings, filters, searchMode: "exact" };
  }

  // ── 2. Destination-based search (itinerary or explicit dest) ──
  if (destination && (filters.isItinerary || explicitDestination)) {
    // Mongo regex on location + country — fast and precise for city names
    const destRegex = new RegExp(destination, "i");
    let destListings = await Listing.find(
      {
        $or: [
          { location: destRegex },
          { country:  destRegex },
    ...
    // If mongo regex finds nothing, fall back to vector search with destination
    if (destListings.length === 0) {
      destListings = await vectorSearch(destination, 6, priceFilter);
    }

    if (destListings.length > 0) {
      filters._destMatched = true;
      return { listings: destListings, filters, searchMode: "destination" };
    }

    // Destination genuinely not in DB — load all so AI can acknowledge it
    filters._noDestMatch = true;
    const all = await Listing.find({}, LISTING_FIELDS).lean();
    return { listings: all, filters, searchMode: "no_dest_match" };
  }

  // ── 3. Price-only filter (no semantic intent) ──
  const hasPriceOnly =
  ...
  if (hasPriceOnly) {
    ...
    return { listings, filters, searchMode: "price" };
  }
  ...
  if (filters.showAll) {
    ...
    if (filters.sort === "cheapest") listings.sort((a, b) => a.price - b.price);
    if (filters.sort === "priciest") listings.sort((a, b) => b.price - a.price);
    return { listings, filters, searchMode: "all" };
  }

  // ── 5. Intent / semantic query — ALWAYS use vector search ──
  // Handles: "solo travel stays", "beach vibes", "romantic getaway",
  //          "luxury villa with pool", "budget stays in Asia" — everything
  ...
  return { listings, filters, searchMode: "vector" };
}

// ─────────────────────────────────────────────
// STEP 4 — Dual-mode system prompt
// ─────────────────────────────────────────────
...
- Countries available: ${allStats.countries.join(", ")}`;
  ...
  // When destination matched listings in DB, label them clearly
...
${listings.map((l, i) =>
  ...
  // Note for when we searched by destination but found nothing
