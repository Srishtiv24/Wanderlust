...
function getBudgetValue() {
  ...
  if (custom)
    return "₹" + parseInt(custom).toLocaleString("en-IN") + "/day custom budget";
  var pill = document.querySelector("#budgetPills .single-sel");
  return pill ? pill.dataset.val : "moderate";
}

/* ── Destination cleaner ── */
/* Strips trailing country/state parts and collapses whitespace      */
/* "Manali, Himachal Pradesh, India" → "manali"                      */
/* "Goa, India" → "goa"                                              */
/* "Manali" → "manali"                                               */
function cleanDestination(raw) {
  return raw
    .split(",")[0]          // keep only the first comma-segment (the city)
    .trim()
    .toLowerCase()
...
function getSuggestions(dest) {
  ...
  if (/mountain|hill|manali|shimla|darjeeling|ooty|munnar|coorg|banff|aspen/.test(d))
...
function scrollChipsLeft() {
  document.getElementById("suggestionChips").scrollBy({ left: -260, behavior: "smooth" });
}
...
function renderDayTabs() {
  ...
  for (var i = 0; i < builderDays; i++) {
    var total = dayPlans[i]
      ? dayPlans[i].morning.length + dayPlans[i].afternoon.length + dayPlans[i].evening.length
      : 0;
    html +=
      '<button class="itin-day-tab' +
      (i === activeDayIdx ? " active" : "") +
      '" onclick="switchDay(' + i + ')">Day ' + (i + 1) +
      (total > 0
...
function renderActiveDay() {
  var dest = document.getElementById("setupDest").value.trim();
  var dp   = dayPlans[activeDayIdx] || { morning: [], afternoon: [], evening: [] };
  ...
  var slots = [
    { key: "morning",   label: "Morning",   time: "6 AM–12 PM",  cls: "morning",   icon: "fa-solid fa-sun"       },
    { key: "afternoon", label: "Afternoon", time: "12 PM–6 PM",  cls: "afternoon", icon: "fa-solid fa-cloud-sun" },
    { key: "evening",   label: "Evening",   time: "6 PM–10 PM",  cls: "evening",   icon: "fa-solid fa-moon"      },
  ];
  var slotsHtml = slots.map(function(s) {
    var acts  = dp[s.key] || [];
    var cards = acts.map(function(a, ai) {
      return (
        '<div class="itin-activity" draggable="true" ondragstart="actDragStart(event,' +
        ai + ",'" + s.key + "')\"><span class=\"itin-activity-icon\">" + a.icon +
        "</span><div class=\"itin-activity-text\">" + a.label +
        (a.sub ? '<div class="itin-activity-sub">' + a.sub + "</div>" : "") +
        "</div><button class=\"itin-activity-del\" onclick=\"removeAct(" +
        activeDayIdx + ",'" + s.key + "'," + ai +
        ")\"><i class=\"fa-solid fa-xmark\"></i></button></div>"
      );
    }).join("");
    return (
      '<div class="itin-slot-col">' +
      '<div class="itin-slot-head"><div class="itin-slot-icon ' + s.cls +
      '"><i class="' + s.icon + '"></i></div><span class="itin-slot-title">' +
      s.label + '</span><span class="itin-slot-time">' + s.time + "</span></div>" +
      '<div class="itin-activities">' + cards + "</div>" +
      '<div class="itin-drop-zone" ondragover="dzOver(event)" ondragleave="dzLeave(event)" ondrop="dzDrop(event,' +
      activeDayIdx + ",'" + s.key + "')\">+ Drop here</div>" +
      '<div class="itin-add-activity"><input class="itin-add-input" id="ai-' + s.key +
      '" placeholder="Type &amp; press Enter…" onkeydown="addEnter(event,' +
      activeDayIdx + ",'" + s.key + '\')"><button class="itin-add-btn" onclick="addManual(' +
      activeDayIdx + ",'" + s.key + '\')"><i class="fa-solid fa-plus"></i></button></div>' +
      "</div>"
    );
  }).join("");
  ...
  document.getElementById("dayCanvases").innerHTML =
    ...
    '<div class="itin-canvas-day-num">' + (activeDayIdx + 1) + "</div>" +
    '<div class="itin-canvas-day-info"><strong>Day ' + (activeDayIdx + 1) +
    (dest ? " — " + dest : "") + "</strong><span>" +
    ...
    activeDayIdx + ')"><i class="fa-solid fa-trash"></i> Clear</button></div>' +
    "</div>" +
    '<div class="itin-slots-grid">' + slotsHtml + "</div>" +
    "</div>";
  updateGenBar();
}
...
function updateGenBar() {
  ...
  for (var i = 0; i < builderDays; i++)
    if (dayPlans[i])
      total += dayPlans[i].morning.length + dayPlans[i].afternoon.length + dayPlans[i].evening.length;
  var dest = document.getElementById("setupDest").value.trim();
  document.getElementById("genBarTitle").textContent = total === 0
    ? "Ready to generate?"
    : total + " activit" + (total === 1 ? "y" : "ies") + " across " +
      builderDays + " day" + (builderDays === 1 ? "" : "s");
  document.getElementById("genBarSub").textContent = total === 0
...
async function loadStays(dest) {
  ...
  var grid    = document.getElementById("staysGrid");
  var label   = document.getElementById("staysDestLabel");
  ...
  try {
    // Use cleaned destination for the API search
    var destClean = cleanDestination(dest);
    var res = await fetch("/api/tb-listings?dest=" + encodeURIComponent(destClean));
    if (!res.ok) throw new Error("Failed to fetch stays");

    var data     = await res.json();
    var listings = (data.listings || []).slice(0, 6);
    ...
    grid.innerHTML = listings.map(function(l) {
      ...
      var price    = l.price    || 0;
      return (
        '<div class="itin-stay-card" draggable="true"' +
        ' ondragstart="stayDragStart(event,\'' + title + "','" + location +
        "','" + country + "'," + price + ')" title="Drag to add as stay">' +
        (l.image && l.image.url
        ...
        '<div class="itin-stay-card-loc"><i class="fa-solid fa-location-dot" style="color:#fe424d;font-size:.58rem;margin-right:2px;"></i>' +
        location + ", " + country + "</div>" +
        '<div class="itin-stay-card-price">₹' + price.toLocaleString("en-IN") + "/night</div>" +
        "</div></div>"
      );
    }).join("");
...
function generateFromBuilder() {
  ...
  var styles = [].slice
    ...
    .map(function(el) { return el.dataset.val; });
  ...
  for (var i = 0; i < builderDays; i++) {
    ...
    var fmt = function(s) {
      return s.map(function(a) { return a.label; }).join(", ") || "flexible";
    };
    lines.push(
      "Day " + (i + 1) + ":\n  Morning: " + fmt(dp.morning) +
      "\n  Afternoon: " + fmt(dp.afternoon) +
      "\n  Evening: "   + fmt(dp.evening)
    );
  }
  ...
  // Use cleaned destination as the search key so backend finds listings correctly
  var destClean = cleanDestination(dest);

  window.location.href =
    "/ai-assistant?prompt=" + encodeURIComponent(prompt) +
    "&destination="         + encodeURIComponent(destClean);
}
...
function quickGenerate() {
  ...
  if (date)      prompt += " starting " + date;
  prompt += " for " + group + " (" + size + " people) on a " + budget + " budget.";
  ...
  prompt +=
    " Give a detailed day-by-day itinerary with morning, afternoon and evening activities," +
    " restaurant recommendations, hidden gems, estimated costs in INR, and practical travel tips.";

  var btn = document.getElementById("qGenBtn");
  btn.disabled   = true;
  btn.innerHTML  = '<i class="fa-solid fa-circle-notch fa-spin"></i> Sending to AI…';

  // Use cleaned destination as the search key
  var destClean = cleanDestination(dest);
