...
document
  .getElementById("delConfirmOverlay")
...
function ownerPreview(files) {
  ...
  if (list.length) {
    count.textContent = `${list.length} file${
      list.length !== 1 ? "s" : ""
    } selected`;
    btn.disabled = false;
    btn.style.opacity = "1";
...
function reviewPreview(files) {
  ...
  Array.from(files)
    .slice(0, 5)
    .forEach((f) => preview.appendChild(makePrevThumb(f)));
}
...
function initMap() {
  ...
  let d;
  try {
    d = JSON.parse(raw.textContent.trim());
  } catch (err) {
    console.error("map-data parse error", err);
    return;
  }
  ...
  const isFallback = (la, ln) =>
    Math.abs(la - DELHI.lat) < 0.001 && Math.abs(ln - DELHI.lng) < 0.001;
  ...
  fetch(
    ...
    .then((res) => {
      if (res && res.length > 0)
        renderMap(parseFloat(res[0].lat), parseFloat(res[0].lon));
...
function waitForLeaflet(cb, tries) {
  ...
  if (tries > 40) {
    console.warn("Leaflet never loaded");
    return;
  }
  setTimeout(function () {
    waitForLeaflet(cb, tries + 1);
  }, 150);
}
